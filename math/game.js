...
function endDrag(){drag=null;document.querySelectorAll('.drag-ghost').forEach(g=>g.remove());}
document.addEventListener('pointerdown',e=>{const source=e.target.closest('#crop-source,[data-coin]');if(drag&&drag.captured&&!drag.source.hasPointerCapture(drag.id))endDrag();if(!source||e.button!==0||source.disabled||drag)return;if(source.id==='crop-source'&&(phase!=='packing'||mode!=='manual'))return;drag={id:e.pointerId,x:e.clientX,y:e.clientY,type:source.id==='crop-source'?'crop':'coin',value:Number(source.dataset.coin),ghost:null,source,captured:false};try{source.setPointerCapture(e.pointerId);drag.captured=source.hasPointerCapture(e.pointerId);}catch{}source.addEventListener('lostpointercapture',()=>{if(drag&&drag.id===e.pointerId)endDrag();},{once:true});});
...
document.addEventListener('pointerup',e=>{if(!drag||e.pointerId!==drag.id)return;const d=drag;endDrag();if(!d.ghost)return;const target=document.elementFromPoint(e.clientX,e.clientY);if(d.type==='crop'){const bag=target?.closest('[data-basket]');if(bag)fillBag(Number(bag.dataset.basket));}else if(target?.closest('#change-tray'))addCoin(d.value);suppressClick=true;setTimeout(()=>suppressClick=false,0);});
document.addEventListener('pointercancel',e=>{if(drag&&e.pointerId!==drag.id)return;endDrag();});
